...
class encoder_block(nn.Module):
    def __init__(self, in_c, out_c, proba_dropout, margin_size):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        p = self.pool(x)
        p = self.dropout(p)
        bridge =  x[:,:,self.margin_size:-self.margin_size,self.margin_size:-self.margin_size]
...
class UNet(nn.Module):
    def __init__(self, proba_dropout, proba_dropout_middle):
        ...
        """ Encoder """
        self.e1 = encoder_block(3, 64, proba_dropout, 88)
        self.e2 = encoder_block(64, 128, proba_dropout, 40)
        self.e3 = encoder_block(128, 256, proba_dropout, 16)
        self.e4 = encoder_block(256, 512, proba_dropout, 4)
