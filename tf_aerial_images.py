...
def extract_data(filename, num_images):
    ...
    N_PATCHES_PER_IMAGE = (IMG_WIDTH/IMG_PATCH_SIZE) * \
        (IMG_HEIGHT/IMG_PATCH_SIZE)

    img_patches = [img_crop(imgs[i], IMG_PATCH_SIZE, IMG_PATCH_SIZE)
                   for i in range(num_images)]
    data = [img_patches[i][j]
            for i in range(len(img_patches)) for j in range(len(img_patches[i]))]
...
def extract_labels(filename, num_images):
    ...
    gt_patches = [img_crop(gt_imgs[i], IMG_PATCH_SIZE, IMG_PATCH_SIZE)
                  for i in range(num_images)]
    data = numpy.asarray([gt_patches[i][j] for i in range(
    ...
    with tf.Session() as s:
        ...
        if RESTORE_MODEL:
            ...
            for iepoch in range(num_epochs):
                ...
                #save_path = saver.save(s, 'tf_aerial/tf_aerial_model')
                #print("Model saved in file: %s" % save_path)

        print("Running prediction on submission set")
        prediction_sub_dir = "predictions_submission/"
        if not os.path.isdir(prediction_sub_dir):
            os.mkdir(prediction_sub_dir)

        image_filenames = []
        for i in range(1, SUBMISSION_SIZE + 1):
            filename = f'data/test_set_images/test_{i}/test_{i}.png'
            image_filenames.append(filename)
            img = mpimg.imread(filename)
            pimg = get_prediction(img)
            pimg = from_mask_to_img(pimg)
            pimg.save(prediction_sub_dir + f'prediction_{i}.png')

        masks_to_submission('tf_aerial_submission', *image_filenames)
